...
async function fetchAllSignSubcaseUris () {
  const queryString = `
...
WHERE {
  ...
  return parseSparqlResults(response).map(r => r.signSubcase);
}

async function syncAllSignflowStatuses () {
  const signSubcaseUris = await fetchAllSignSubcaseUris();
  for (const signSubcaseUri of signSubcaseUris) {
    await syncStatusForSignSubcase(signSubcaseUri);
  }
  return signSubcaseUris.length;
}
