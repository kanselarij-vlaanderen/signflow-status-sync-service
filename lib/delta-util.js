function deletionDeltas (deltaBody) {
  return deltaBody.map(d => d.deletes).reduce((ds, d) => Array.prototype.concat.apply(ds, d));
}

function insertionDeltas (deltaBody) {
  return deltaBody.map(d => d.inserts).reduce((ds, d) => Array.prototype.concat.apply(ds, d));
}
